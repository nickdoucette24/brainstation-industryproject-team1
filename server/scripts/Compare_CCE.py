...
# The followings are to prepare the date variable
...
df = pd.merge(index, cce, how = "left", on = ['CCE_sku'])
df = pd.merge(df, dell, how = "left", on = ['Dell_product'])
df = df[['Dell_product', 'CCE_sku', 'CCE_price', 'Dell_price']]
...
df['Price_dif'] = df['CCE_price'] - df['Dell_price']
...
plt.xticks(rotation=-45)
plt.title('The deviation % of Canada Computers & Electronics\'s price comparing to Dell MSRP')
...
display(df[['Dell_product', 'Dell_price', 'CCE_price', 'Deviation', 'Status']].sort_values('Deviation', ascending = True))
...
#save = df[['Dell_product', 'Dell_price', 'CCE_price', 'Deviation', 'Status']].sort_values('Deviation', ascending = True)
#save.to_csv(f'cce_comparison{date}.csv')
